...
TEST_CASE("test_constant_int", "[constant_reactor]") {
  ...
  REQUIRE(constant->commit(0) == base_reactor::update::COMPLETE_WITH_EVAL);
...
TEST_CASE("test_constant_decimal", "[constant_reactor]") {
  ...
  REQUIRE(constant->commit(100) == base_reactor::update::COMPLETE_WITH_EVAL);
...
TEST_CASE("test_constant_string", "[constant_reactor]") {
  ...
  REQUIRE(constant->commit(123) == base_reactor::update::COMPLETE_WITH_EVAL);
...
TEST_CASE("test_constant_lift", "[constant_reactor]") {
  auto literal = lift(5);
  REQUIRE(literal->commit(0) == base_reactor::update::COMPLETE_WITH_EVAL);
  REQUIRE(literal->eval() == 5);
  REQUIRE(literal->commit(1) == base_reactor::update::NONE);
  auto reactor = lift(make_constant_reactor(10));
  REQUIRE(reactor->commit(0) == base_reactor::update::COMPLETE_WITH_EVAL);
